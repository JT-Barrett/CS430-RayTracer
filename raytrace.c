...
int raycast(Pixel *buffer, Scene scene, int num_objects, int width, int height){
  ...
  for (int y = 0; y < N; y += 1) {
    for (int x = 0; x < M; x += 1) {
      ...
      double best_t;
      int best_obj;
      shoot_ray(scene, num_objects, Ro, Rd, 0, &best_t, &best_obj);
...
|Intersect Methods
\*********************/
//This function checks for a ray intersection with a sphere
double sphere_intersection(double* Ro, double* Rd, double* C, double r, int option){
...
//This function checks for a ray intersection with a plane
...
|Helper Methods
\*********************/
// this function shoots a ray from a location given a direciton and returns the distance to a hit and the object index hit in outmode
void shoot_ray(Object *scene, int num_objects, double *Ro, double *Rd, int obj_origin, double *t, int *o){
  v3_normalize(Rd);
  ...
  for (int i = 1; i < num_objects; i += 1) {
    ...
    switch (scene[i].kind) {
    case 1:
      t = sphere_intersection(Ro, Rd, scene[i].sphere.center, scene[i].sphere.radius, 0);
      break;
    case 2:
    ...
    if (t > 0 && t < best_t && best_t > 0.001 && i != obj_origin){
      best_t = t;
      best_obj = i;
    }
  }
...
//Shooting a ray to check for a shadow is a bit different, so it gets it's own function. In most ways it does the same as the previous.
void shoot_ray_shadow(Object *scene, int num_objects, int best_obj, double *Ro, double *Rd, double *t, int *o){
  ...
  for (int k=1; k < num_objects ; k += 1){
    ...
    if (k != best_obj){
      ...
      switch(scene[k].kind){
      case 1:
        t_shadow = sphere_intersection(Ro, Rd, scene[k].sphere.center, scene[k].sphere.radius, 0);
        break;
      case 2:
      ...
      if (t_shadow > 0 && t_shadow < best_t_shadow && best_t_shadow > 0.001 && best_obj_shadow != best_obj){
...
//This is the recursive workhorse for the raytracer, it shoots additional rays and shades back recursively
  ...
  for (int i = 1; i < num_objects; i += 1) {
    ...
    if (best_t_shadow > 0 && best_t_shadow != INFINITY && best_t_shadow < dist ) {
      ...
      if(scene[i].light.theta != 0){
        ...
        ang_a = 1;
      color[0] += (rad_a * ang_a * (diff[0] + spec[0])) * (1 - scene[best_obj].refractivity);
      color[1] += (rad_a * ang_a * (diff[1] + spec[1])) * (1 - scene[best_obj].refractivity);
      color[2] += (rad_a * ang_a * (diff[2] + spec[2])) * (1 - scene[best_obj].refractivity);
      ...
      if (level < MAX_REC) {
        if(scene[best_obj].reflectivity > 0){
          ...
          //shoot the next ray to ge the next object and distance
          double rec_t = INFINITY;
          int rec_obj;
          shoot_ray(scene, num_objects, Ron, refl_ray, best_obj, &rec_t, &rec_obj);

          //recursively add color to the pixel based on what was hit.
          double refl_col
          or[3] = {0, 0, 0};
          if (rec_t != INFINITY && rec_obj != best_obj && rec_t > 0.001)
            shade_rec(scene, num_objects, rec_t, rec_obj, Ron, refl_ray, refl_color, level + 1);
          v3_scale(refl_color, scene[best_obj].reflectivity, refl_color);
          v3_add(refl_color, color, color);
        }

        // this is an attempt to add refractivity, it was not successful and all objects would just appear darker
        /*
        if(scene[best_obj].refractivity > 0){
          double a[3]; //cross
          double b[3]; //tangent to surface
          double ut[3];//final refracted ray
          double Rd_ior[3]; //intermediary storage
          double ncos[3];
          double bsin[3];
          double color_transp[3];
          ...
          double ior = 1/scene[best_obj].ior;
          v3_scale(Rd, ior, Rd_ior);
          double sin_phi = v3_dot(Rd_ior, b);
          double cos_phi = sqrt(1-sqr(sin_phi));

          //ut = -n*cos_phi + b*sin_phi
          v3_scale(N, -1*cos_phi, ncos);
          v3_scale(b, sin_phi, bsin);
          v3_add(ncos, bsin, ut);
          v3_normalize(ut);

          double rec_t = INFINITY;
          int rec_obj;
          shoot_ray(scene, num_objects, Ron, ut, best_obj, &rec_t, &rec_obj);

          double refr_color[3] = {0,0,0};
          if (rec_t != INFINITY && rec_t > 0.001)
            shade_rec(scene, num_objects, rec_t, rec_obj, Ron, ut, refr_color, level + 1);
          v3_scale(refr_color, scene[best_obj].refractivity, refr_color);
          v3_add(refr_color, color, color);
        }*/
      }
    }
  }
